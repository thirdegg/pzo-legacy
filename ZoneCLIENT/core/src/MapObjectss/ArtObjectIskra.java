 ...
 */
public class ArtObjectIskra extends MapObject {
    Animation anim;
    float stateTime;
    public float timevis;

    public ArtObjectIskra(float xx, float yy, long id) {
        super("iskra.png", xx, yy, 2,null,false);
        this.id = id;
        tip = Util.ARTISKRA;
        TextureRegion[][] tmp = split(getRegionWidth()/3,getRegionHeight());
        int index = 0;
        TextureRegion[] tanim = new TextureRegion[3];
        for (int j = 0; j < 3; j++) {
            tanim[index++] = tmp[0][j];
        }
