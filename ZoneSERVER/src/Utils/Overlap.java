...
public class Overlap {
    ...
    public static boolean pointPoint(float x, float y, float x1, float y1, int rastoynie) {
        float distX = x - x1;
        float distY = y - y1;
        float tmp = distX * distX + distY * distY;
        return tmp < rastoynie * rastoynie;
    }
    ...
    public static boolean overlapPointRectang(float x,float y,Rectang r){
        return x>r.x&&x<r.width+r.x&&y>r.y&&y<r.height+r.y;
    }
}
