...
import MapObjects.Artefakts.Heart;
import MapObjects.Artefakts.Heart1;
import MapObjects.Artefakts.Heart2;
...
/*
аномалия импульс не видина для игрока если игрок соприкоснется с ней то его отброит в
определенном направлении и у него онимится определенное кол-во здоровья
 */
public class Impuls extends MapObject {
    ...
    public Impuls(int x, int y, HashMap<Long, MapObject> mapobjects, int napravlenie, List<AddMOparameters>listaddobject, SpatialHashGrid grid) {
        // создаем новый id
        ...
        if(rnd>=4){
            ...
            if(rnd==0)heart=new Heart(x,y,grid,this);
            if(rnd==1||rnd==2)heart=new Heart1(x,y,grid,this);
            listaddobject.add(new AddMOparameters(heart,false,false,0));}
    }
    @Override
        ...
        switch(state){
            ...
            case 1:
                ...
                if (timezarad > 7) {
                    ...
                    atakobject.signal(4,new AnomalyJump(atakobject,znaprav,80,10,false));}
                    state=2;
                    atakobject=null;
                    timezarad = 0;
                }
    ...
    public void signal(int tip,Object object) {
        switch (tip) {
            ...
            case  8:
                ...
                break;
            case 9:
                if(heart!=null&&(StateWorld.hours>=0||StateWorld.hours<9)){
                heart.clientvisible=true;
                sendMsgPlayers("1/" +heart.tip + "/" + heart.id + "/" + (int) x + "/" + (int) y+"/"+heart.napravlenie);}
                break;
        }
    }
